...
class DiscriminatorTrainer:
    ...
    def train(self, discriminator, generator):
        ...
        for i in range(self.__iterations):
            ...
            generated_images = self.__prepare_data_for_discriminator(generator, x_train)
            # positive
            positive_labels = np.array([self.__label_weight] * self.__batch_size)
            discriminator.train_on_batch(x_train, positive_labels)
            # negative
            negative_labels = np.zeros(self.__batch_size)
            discriminator.train_on_batch(generated_images, negative_labels)
